...
# ═════════════════════════════════════════════════════════════════════
# 6. API จากแอปหลัก (กล้อง + YOLO + คำนวณระยะ)
# ═════════════════════════════════════════════════════════════════════
...
# 🔥 จุดสำคัญ: ระบบ Tracker กรองผ่านชื่อคลาส 's', 'c', 'b' แบบไม่ต้องสน Index
...
def detect_red_ball():
    ...
    img = decode_image(data['image'])
    # ไม่ต้องใส่ classes=[...] ปล่อยให้มันหาทั้งหมดก่อน
    ...
    if len(results) > 0:
        for box in results[0].boxes:
            ...
            # ดึงชื่อคลาสออกมาตรวจสอบ
            cls_id = int(box.cls[0])
            class_name = results[0].names[cls_id].lower()
            
            # 🚀 ถ้าชื่อคลาสคือ s, c, b และขนาดรัศมี 5-80 พิกเซล ถึงจะเอา!
            if class_name in ['s', 'c', 'b' , 'g'] and 5 <= r <= 80:
                balls.append({
                    "x": float(cx), 
                    "y": float(cy), 
                    "radius": float(r), 
...
# 🔥 จุดสำคัญ: ระบบกล้องสด (Live Tester) ก็กรองผ่านชื่อคลาส 's', 'c', 'b' เช่นกัน
    ...
    try:
        ...
        if len(results) > 0:
            for box in results[0].boxes:
                ...
                # 🚀 กรองให้โชว์กรอบบนหน้าเว็บเฉพาะ s, c, b
                if name in ['s', 'c', 'b' , 'g']:
                    boxes_data.append({
                        "xmin": xyxy[0], 
                        "ymin": xyxy[1],
